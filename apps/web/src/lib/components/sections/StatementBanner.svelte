...
<div class="checker-red-yellow"></div>
...
>
  ...
  <Section id={block.id ?? ""} class="items-center">
    <!-- Product image -->
    {#if src}
    ...
    <div class="flex-1 flex flex-col gap-4 text-center uppercase">
      {#if block.headline}
        <h2
          class="font-script text-brand-cream text-[72px] leading-[0.84] whitespace-pre-line"
        >
      ...
      {#if block.subtitle}
        <p
          class="font-display font-black text-brand-yellow text-[12px] tracking-[5px]"
        >
          {block.subtitle}
        </p>
      {/if}
